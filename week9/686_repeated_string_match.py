...
class Solution:
    ...
    def repeatedStringMatch(self, a: str, b: str) -> int:
        ...
        b 属于 a 叠加子串的 共三种情况, 没有第四种情况, 再加 a 没有意义:
            1. k * a == b (叠加后的 a 字符串刚好和 b 长度一样, 并且完全匹配)
            2. k * a + a == b (叠加后的 a 字符串比 b 长, 但没有超出 len(a) 的长度, 此时 b 是 叠加 a 字符串 的 子串)
            3. k * a + a + a == b (叠加后的 a 字符串比 b 长, 并超出 len(a) 的长度, 此时 b 是 叠加 a 字符串 的 子串)
        """
        ans1 = ""
        k = len(b) // len(a)
        for _ in range(k):
